...
app.post('/api/execute-tool', async (req, res) => {
    ...
    if (toolName !== 'run_terminal_command') {
        return res.status(501).json({ status: "Error", message: `Tool '${toolName}' is not implemented on the backend.` });
    }

    const { command } = parameters;
    if (!command) {
        return res.status(400).json({ status: "Error", message: "A 'command' parameter is required." });
    }
    ...
    ptyProcess.onExit(({ exitCode, signal }) => {
        ...
        if (exitCode === 0) {
            ...
            res.status(500).json({ status: "Error", message: `Command failed with exit code ${exitCode}.`, output: output });
        }
    });
    ...
    setTimeout(() => {
        if (!res.headersSent) {
            ...
            res.status(500).json({ status: "Error", message: "Command execution timed out.", output: output });
        }
